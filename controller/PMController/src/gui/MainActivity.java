...
public class MainActivity extends Activity {
    ...
    public void onDestroy() {
        if (DEBUG)
            Toast.makeText(getApplicationContext(), "deattaching NSD service", Toast.LENGTH_SHORT).show();
        // Teardown the service
        if (server != null) {
            destroyServer();
        }
        if (DEBUG)
            Toast.makeText(getApplicationContext(), "destroid Main", Toast.LENGTH_SHORT).show();
    ...
    protected void onResume() {
        if (DEBUG)
            Toast.makeText(getApplicationContext(), "on resume main", Toast.LENGTH_SHORT).show();
    ...
    void handleWifi() {
        ...
        if (server == null) {
            ...
            if (mWifi.isConnected()) {
                ...
                if (DEBUG)
                    Toast.makeText(getApplicationContext(), "Start Server, enabled button", Toast.LENGTH_SHORT).show();
                setConnectionText(0);
                show_connected = true;
            ...
            if (!mWifi.isConnected()) {
                ...
                if (server != null) {
                    ...
                    start_button.setEnabled(false);
                    if (DEBUG)
                        Toast.makeText(getApplicationContext(), "Detached Server, disabled button", Toast.LENGTH_SHORT).show();
                    showed_alert = false;
                }
            } else {
                if (!show_connected) {
